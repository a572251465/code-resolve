...
export async function resolveConfig(
  ...
  // run config hooks
  // 按照顺序执行插件 直接插件中的config钩子 对配置进行覆盖
  ...
  // load .env files
  // 此处允许加载不同的env 可以自定义路径
  const envDir = config.envDir
    ? normalizePath(path.resolve(resolvedRoot, config.envDir))
    : resolvedRoot
  // 此处加载几种不同的配置文件 .env .env.local .env.development.local
  const userEnv =
